...
const Background3d: React.FC<Background3dProps> = ({ theme, zoomLevel }) => {
  ...
  useEffect(() => {
    ...
    // ---- Sphere Shader ----
    const colorA = new THREE.Color(theme === "dark" ? 0x00aaff : 0x007acc);
    const colorB = new THREE.Color(theme === "dark" ? 0xffffff : 0x1f2937);
    uniforms.current = {
      time: { value: 0 },
      mouseX: { value: 0 },
      mouseY: { value: 0 },
    ...
    const material = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          float baseWave = sin(time*0.5+position.x*freq)*amp+
                           cos(time*0.3+position.y*(freq+1.5))*amp*0.75+
                           sin(time*0.7+position.z*(freq+0.5))*amp*0.5;
          float meltScale = mix(1.5,3.0,uScroll);
          float melt = snoise(position*(1.2+0.6*uScroll)+time*0.06)*0.06*meltScale;
          vec3 newPos = position + normal*(melt+baseWave);
          if(position.y < -0.2){
            float dripStrength = smoothstep(-0.2,-1.5,position.y)*(0.6+0.8*uScroll);
      ...
      fragmentShader: `
        ...
        ${noiseGLSL}
        void main() {
          float n = snoise(vPos*(2.0+uScroll*2.0)+time*(0.02+0.04*uScroll)+mouseX*0.4+mouseY*0.4);
          float mask = smoothstep(-0.25,0.35+0.15*uScroll,n);
          vec3 base = mix(uColorA,uColorB,uScroll);
          vec3 color = mix(vec3(0.0),base,mask);
          color *= (1.0+0.2*uScroll);
    ...
    const animate = () => {
      ...
      uniforms.current.uScroll.value = s;
  ...
  }, [theme, zoomLevel, handleMouseMove]);
